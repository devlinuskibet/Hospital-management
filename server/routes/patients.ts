...
router.post(
  "/",
  authenticateToken,
  authorizeRoles(UserRole.ADMIN, UserRole.RECEPTIONIST, UserRole.NURSE),
  validatePatientRegistration,
  handleValidationErrors,
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (existingPatient) {
        return res
          .status(400)
          .json({ error: "Patient with this National ID already exists" });
      }
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      res.json({
        ...
        nhifCoverage:
          totalPatients > 0
            ? ((patientsWithNHIF / totalPatients) * 100).toFixed(1)
            : 0,
      });
    } catch (error) {
